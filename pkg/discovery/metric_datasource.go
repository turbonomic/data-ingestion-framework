...
func loadJSONStream(metricEndpoint string, resp []byte) (*data.Topology, error) {
	...
	for json := range parser.Stream() {
		...
		entity := &data.DIFEntity{
			...
			PartOf:              nil,
			Metrics:             nil,
			ProviderMustClone:   json.ObjectVals["providerMustClone"].BoolVal,
			Controllable:        json.ObjectVals["controllable"].BoolVal,
			Cloneable:           json.ObjectVals["cloneable"].BoolVal,
			Suspendable:         json.ObjectVals["suspendable"].BoolVal,
		}
